...
namespace etcd
{
  ...
  public:
     ...
     */
      ETCD_API Client(std::string const & etcd_url);

    /**
     * Sends a get request to the etcd server
     * @param key is the key to be read
     */
      ETCD_API pplx::task<Response> get(std::string const & key);
     ...
     */
      //ETCD_API pplx::task<Response> set(std::string const & key, std::string const & value);
      ...
      * @param key is the key to be created or modified
      * @param value is the new value to be set
      * @param ttl indicates keys in etcd can be set to expire after a specified number of seconds
      * @param refresh indicates keys in etcd can be refreshed without notifying current watchers and it can be achieved by setting the refresh to true when updating a TTL
      */
      ETCD_API pplx::task<Response> set(std::string const & key, std::string const & value, int ttl = 0, bool refresh = false);
     ...
     */
      ETCD_API pplx::task<Response> add(std::string const & key, std::string const & value);
     ...
     * @param value is the new value to be set
     */
      ETCD_API pplx::task<Response> modify(std::string const & key, std::string const & value);
     ...
     */
      ETCD_API pplx::task<Response> modify_if(std::string const & key, std::string const & value, std::string const & old_value);
     ...
     */
      ETCD_API pplx::task<Response> modify_if(std::string const & key, std::string const & value, int old_index);

    /**
     * Removes a single key. The key has to point to a plain, non directory entry.
     * @param key is the key to be deleted
     */
      ETCD_API pplx::task<Response> rm(std::string const & key);
     ...
     * @param key is the key to be deleted
     */
      ETCD_API pplx::task<Response> rm_if(std::string const & key, std::string const & old_value);
     ...
     */
      ETCD_API pplx::task<Response> rm_if(std::string const & key, int old_index);

    /**
     * Gets a directory listing of the directory identified by the key.
     * @param key is the key to be listed
     */
      ETCD_API pplx::task<Response> ls(std::string const & key);
     ...
     */
      ETCD_API pplx::task<Response> mkdir(std::string const & key);
     ...
     */
      ETCD_API pplx::task<Response> rmdir(std::string const & key, bool recursive = false);
     ...
     */
      ETCD_API pplx::task<Response> watch(std::string const & key, bool recursive = false);
     ...
     * @param recursive if true watch a whole subtree
     */
      ETCD_API pplx::task<Response> watch(std::string const & key, int fromIndex, bool recursive = false);
